...
- [x] TASK-125 — `metrics_scan` re-cut (amended FR-MX-02): `core/scripts/metrics_scan.py` rebuilt on the ADR-008 vocabulary — M01 $/SI-v1, M02 $/enrichment (**both** `enrichment` *and* `si_v2` — the apply pass is enrichment's cost, not a separate one), M03 score at each accepted gate, M04 first-pass, M05 docs/month, M06 v1→v2 cycle, M07 p95, M09 §16→story coverage, M10 stories/epic, M11 push success, and **M12 enrichment yield** — the v1→v2 delta, three counts off `verdict.route`, which is what that field was put on the event for. Every metric is computed **per run then averaged, never pooled** (pooling lets one long run stand in for the fleet, and M09 is a per-run score); a metric with no events yields **`None`, never 0** — a run that never pushed has no success *rate*, and 0% would be a claim. M12 is the deliberate exception: a run that enriched and yielded nothing yielded 0, and that is a finding. **Two gaps the re-cut surfaced, both fixed:** (1) the amended M10 is stories/**epic** but `jira_push` carried only `epics` — the metric was underivable, so `stories` joins the event (schema + emitter + call sites); (2) `docs/TECH_SPEC.md` §8.1/§8.2 was still the pre-pivot contract — stage list, `validation.artifact`, and the three enrichment events had landed in the ledger schemas at TASK-104/110/121 without ever being carried into the spec — now amended with a **port note**. Proof: 🆕 `fixtures/metrics/verify_metrics.py` — the ledger is **replayed from the D4 record** (`verify_spine.build_findings` through the shipped `emit_finding_events`) with push counts off the D5 plan, so the events are the events the pipeline emits; **36 checks**, each fleet check naming the wrong-but-plausible implementation it kills — three companion runs (G1 reopened + plan refused at G3 + never pushed · ingest-only · push failed) make M09 reject a global "last jira score", M12's denominator exclude a run that never enriched, M04 skip rather than score it 0, and M11 count a failed push while not counting an absent one. M12 off the ledger is matched against the same count taken off the record — two independent paths. §10 4/4; all 26 verifies green.
...
- [x] TASK-125 — `metrics_scan` re-cut (amended FR-MX-02) · `Sonnet`
