...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--demo" in argv or not argv:
        return _demo()
    # Reject unknown flags. argv[0] is taken as the map directory, so an unrecognised `--flag`
    # was read as a PATH — and `--help` therefore crashed with a traceback about a missing
    # components.json instead of printing usage. A silently-misread argument is the same
    # misdirection class as a check aimed at the wrong directory (TASK-127).
    unknown = [a for a in argv if a.startswith("-")]
    if unknown:
        print(f"unknown option(s): {' '.join(unknown)}", file=sys.stderr)
        print("usage: check_map_totality.py <code_map-dir> | --demo", file=sys.stderr)
        return 2
    d = Path(argv[0])
    if not (d / "components.json").is_file() or not (d / "files.json").is_file():
        print(f"not a code_map directory (needs components.json + files.json): {d}",
              file=sys.stderr)
        return 2
